"""Classic PythonOperator sequence DAG.
...
def square_number(**context):
    current_value = context["ti"].xcom_pull(
        key="current_value", task_ids="subtract_three_task"
    )
    new_value = current_value**2
    print(f"Square result: {current_value}^2 = {new_value}")


with DAG(
    dag_id="math_sequence_classic",
    start_date=datetime(2024, 1, 1),
    schedule="@once",
    catchup=False,
    default_args=DEFAULT_ARGS,
    tags=["math", "pipeline", "pythonoperator"],
) as dag:
    ...
    multiply_by_two_task = PythonOperator(
        task_id="multiply_by_two_task", python_callable=multiply_by_two
    )
    subtract_three_task = PythonOperator(
        task_id="subtract_three_task", python_callable=subtract_three
    )
    square_number_task = PythonOperator(
        task_id="square_number_task", python_callable=square_number
    )
